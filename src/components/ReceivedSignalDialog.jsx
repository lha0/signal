...
const ReceivedSignalDialog = ({ closeReceivedSignal }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const result = await receivedSignalFunction(loggedInUserId);
    ...
    return (
        <>
            <Container>
                <Title>Received Signal</Title>
                <ListView>
                    {resultSignals.map((signal) => (
                        <ListItem key={signal.id}>{signal.name}</ListItem>
                    ))}
                </ListView>
                <NoBtn onClick={closeReceivedSignal}>X</NoBtn>
            </Container>
        </>
    );
};

export default ReceivedSignalDialog;
